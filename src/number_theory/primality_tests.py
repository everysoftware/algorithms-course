...
def decompose(n: int) -> tuple[int, int]:
    """Функция разложения числа на нечетное число и степень двойки. Сложность O(log(N))."""
    s = 0
...
def primality_tests(n: int) -> tuple[list[int], int]:
    """Функция проверки числа на простоту с помощью тестов Ферма и Миллера-Рабина. Сложность O(N)."""
    fermat_successful = 0
